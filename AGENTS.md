...
- The OCR sidecar (`services/ocr`, FastAPI on port 8000, reached by the api through `OCR_SERVICE_URL`) sits under the `ocr` compose profile, so a plain `docker compose up -d` never starts it. It builds with `docker compose --profile ocr build ocr` and tests with `docker compose --profile ocr run --rm ocr pytest`; both stay outside `pnpm verify` and run, under the host lock `flock /tmp/fasor-verify.lock`, only for stories touching `services/ocr` or its contract. Its JSON Schema is exported from `packages/domain/src/contract/ocr.ts` with `docker compose --profile tools run --rm tools pnpm schema:ocr`; a `test:unit` kernel test fails when the committed schema drifts.
- Every `test:e2e*` command runs through `scripts/e2e.ts`: it builds the web bundle once, then runs a parallel group and after it a serial group (`export.spec.ts` and `export-visual.spec.ts`, which share the document queue, LibreOffice and a fixed-id fixture, on one worker); both groups always run, the command fails when either failed, and `test-results/e2e-report/summary.json` holds the per-group and total counts with every test's outcome. Each parallel worker works on its own pair of companies (`workerSeed(index)` in `apps/api/src/db/e2e-worker-seed.ts`; the global setup seeds at least 3 pairs), and the leak check (`apps/api/src/db/e2e-leak-check.ts`) runs as the global teardown and fails the run when a worker's user wrote into any company but its own pair. The parallel group's default worker count is `PARALLEL_WORKERS` in `e2e/support/groups.ts` (1; three workers still fail one load-sensitive test per full run, see `deferred-work.md`); `--workers=N` opts into more for one run and reaches the parallel group only. On a shared machine only one gate runs at a time: wrap `verify`, `test:e2e:full`, `test:e2e:matrix` and any multi-worker run in `flock /tmp/fasor-verify.lock`.
