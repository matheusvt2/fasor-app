...
**Then** the emitting device appends `relatorio/status = Em revisão` to the same batch, computed by `statusTable`, and a later edit does not re-advance (FR-21, AR-21) ~~(covers every family, including `equipment`)~~ (narrowed 2026-09-24, batch C review: `equipment` ops are project-scoped (`relatorio_id: null`, TAGs are shared across a project's relatórios) and `buildBatch`'s per-relatório grouping only ever sees relatório-scoped ops on the same batch, so a TAG rename alone never resolves which of the project's relatórios reference the equipment and therefore never advances any of them; every OTHER family in the set is relatório-scoped and correctly covered. Fixing this needs a new cross-relatório query (which relatórios of the project have a block on this equipment) inside `commitBatch`'s hot path, used by every write in the app; deferred rather than risked here, tracked in `deferred-work.md`)
