...
| **Section stepper** | 48px row (`{components.section-stepper}`) at the top of the Sticky action bar, four equal buttons: step name in `label` `ink-secondary` + its count — "Placa 2 · Verif. 0 · Ensaios 1 · Conclusão 1" — the count in `fora-do-limite` while > 0, a ✓ in `conforme` at 0. Current section in `primary` ~~with a 3px `primary` bottom rule~~ (2026-09-26: struck; the v0.9 stepper row, the progress rule "4px on the current", is the one current mark, and the 3px bottom rule drew a second line under it). Names abbreviate on phone ("Verif."), never wrap. |
