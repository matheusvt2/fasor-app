...
**Merge with main (Stories 1.7 and 1.4 landed while this story was built).** Resolved on 2026-09-22 after the workflow completed, keeping both sides' behavior: the identity tables now ship as `apps/api/drizzle/0001_identity.sql` after main's `0000_sweet_solo` (my `apps/api/src/db/migrations/` folder was dropped; the drift test reads `migrationsFolder` from main's `migrate.ts`); `company.id` and every `company_id` of this story are `uuid`, like `ops.company_id`/`entities.company_id`, so `asCompanyId` requires a uuid and the two test companies carry fixed uuids; main's `createDb(url) => { sql, db }`, `Db`, `migrate(db)` and the JSON `log`/`logError` replace `createSql`/`Database`/`runMigrations`; `createApp` takes `{ probes, auth, db, staticDir? }` and keeps main's request logging (now with `company_id` from the session), static serving and SPA fallback beside the auth handler, session middleware, account routes and JSON 404 envelope for `/api/*`; the web uses main's `apps/web/src/db/schema.ts` (`openDatabase`, `databaseName`, `AppDatabase`) instead of the removed `database.ts`, with the sign-out-keeps-the-database test rewritten as `apps/web/src/db/sign-out.test.ts` and the e2e marker row stored in `local_prefs` (main keys `entities` by `[entity+id]`); the compose default of `TRUSTED_ORIGINS` gained the Caddy `https://` origins so sign-in works through Story 1.7's TLS front. Conflicted files: `sprint-status.yaml`, `apps/api/drizzle.config.ts`, `apps/api/package.json`, `apps/api/src/db/{client,migrate,schema}.ts`, `apps/api/src/http/app.ts`, `apps/api/src/main.ts`, `apps/web/package.json`, `package.json`, `pnpm-lock.yaml` (regenerated in the container).
