...
The mandatory independent review (PR #15 comment) filed 7 findings on top of the internal pass: one high, two medium, four low. All seven were patched and re-verified by the same reviewer, whose re-review verdict is approve.

- `[high]` `[patch]` `GET /api/files/{id}/{thumb|print}` took the object key from `row.variants`, which is client-supplied JSON on a client create family, so company B could push a create for its own file carrying company A's variant keys and read A's bytes (reproduced twice against the running stack) — fixed: `GET` derives every key as `objectKey(session.companyId, row.kind, id, variant)` and a missing object is the 404; `PUT` also stopped trusting `row.variants` and reports only the keys it wrote.
- `[medium]` `[patch]` The same client-controlled key reached S3 unvalidated, so `variants: {print: '../../../etc/passwd'}` answered `500` — removed by the fix above, asserted as a 404.
- `[medium]` `[patch]` A client create could carry `uploaded_at` or `variants`, leaving a file permanently "attached but missing" because `pendingUploads` skips a row with `uploaded_at` set — fixed: `apps/api/src/sync/apply.ts` rejects such a client-origin `file` create as `op_invalid` (`reading_status` deliberately excluded; Epic 6 owns its rule).
- `[low]` `[patch]` The body length was never compared with `row.size` — fixed: a mismatch is refused `409 file_sha_mismatch` before anything is stored.
- `[low]` `[patch]` `files_pending` omitted files a phase cut short never attempted — fixed: the count is by subtraction.
- `[low]` `[patch]` A 200 whose body does not parse was classified `permanent`, burning the file for the session — fixed: `invalid_response` is `retry`.
- `[low]` `[patch]` The visually-hidden file input kept an `aria-label`, so every tile exposed a phantom second control — fixed: it is `aria-hidden` with a `data-testid` for tests.
- `[low]` `[patch]` (found in the re-review) `2.2-API-007`'s path-escape case inserted a row without `removed_at` inside the row JSON, so it 404'd on the parse and passed whatever the key branch did — fixed: the row is now parseable and the case is discriminating.

**Merge with `origin/main`** (the parallel Stories 2.4-2.6 batch, PR #14): the tab strip is theirs, untouched here. Three reconciliations were needed. The kernel `index.ts`, `apply.ts`, `home-store.ts`, `components/index.ts` and both CSS files took both sides. The replay golden snapshot was regenerated from the merged op-log fixture, so it carries both batches' schema changes. `checks/pre-issue.ts`'s row type was renamed `CompanyPreIssueRow` because the other batch landed its own, narrower `PreIssueRow` in `checks/pre-issue-client.ts`; Epic 7's aggregator unifies them (recorded in `deferred-work.md`). A union-merged CSS rule had lost a closing brace, which made the production bundle fail to build (lightningcss reported it as "Unknown at rule: @font-face" 400 lines later, masked further by this repo's `shell-precache` plugin, whose `closeBundle` runs on failure too and then fails on the missing `dist/index.html`); fixed and re-verified.

**Post-merge verification.** `docker compose --profile tools run --rm tools pnpm verify` green: lint, static, test:unit, test:api (96 tests) and Playwright `@p0` 21/21 across both batches. `pnpm test:e2e:full` 47 passed, including this story's Empresa cases at 390, 768 and 1280 px against the other batch's new tab strip.
