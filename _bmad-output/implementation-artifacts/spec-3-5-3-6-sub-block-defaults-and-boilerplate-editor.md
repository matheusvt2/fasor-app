...
### 2026-09-23 — Independent review pass (PR #21, fresh context, model opus)
- verdicts: 9 findings — high 0, medium 1, low 8
- findings:
  - `[medium]` `[patch]` the subtype control in `TypeDefaultsDialog` was a native `<select>`, the only one in the app; the mock (`42-template-composer.html`) draws it as the app's own `.field.combobox` shell and AGENTS.md requires behavior from React Aria Components — evidence: `grep -rl "<select" apps/web/src` found exactly one file. Fixed: replaced with the existing `Combobox` component (a `NO_SUBTYPE` sentinel key stands in for an empty React Aria key); the seed's own uppercase subtype labels ("MANUAL", "Á SECO") are kept verbatim, not "corrected" to the mock's illustrative casing.
  - `[low]` `[patch]` the "Restaurar" flush-before-restore fix (this same pass's earlier patch) had no test proving "type then restore within the idle window" actually restores the latest typed text, not an older committed one — evidence: the existing restore test only covered a chip already saved on close. Fixed: a new composer test types, restores at once (inside the 500 ms idle), and asserts "Desfazer" brings back the just-typed text.
  - `[low]` `[patch]` when the section a text dialog was opened on moved/changed elsewhere, the dialog stayed open and repeated the "changed elsewhere" toast on every later autosave attempt — evidence: `onCommitText` showed the toast but never closed the dialog. Fixed: the dialog now closes the first time `gone` is true, mirroring how the type-defaults dialog already auto-closes when its type leaves the template.
  - `[low]` `[patch]` only `formatBold`/`formatItalic` were blocked in the `beforeinput` listener; `formatUnderline` and `formatStrikeThrough` still drew formatting silently dropped on save, misleading the user — evidence: read the prevented `inputType` set. Fixed: both added.
  - `[low]` `[reject]` dropping an equipment type's last placement to zero and re-adding it loses its customized subtype/toggles, reverting to the pristine seed default — verified true (`typeConfigFor` returns null with no placement), but this is the intended "per-type config keyed by current placements" design (Design Notes), not a defect; logged to `deferred-work.md` as an accepted note rather than patched.
  - `[low]` `[reject]` clearing a section's whole text falls back to the seed default with no toast, unlike the explicit "Restaurar" action — consistent with the app's general silent-autosave pattern elsewhere (only explicit destructive/restore actions toast); left as-is, logged to `deferred-work.md`.
  - `[low]` `[reject]` `section-text-dialog.tsx` lowercases the domain's `SECTION_VARIABLE_LABELS` for chip button text in `apps/web` — a presentational capitalization choice on an existing domain label, not new derived business text; a defensible reading of the string-home rule, logged to `deferred-work.md` as a minor note rather than patched.
  - `[low]` `[defer]` Story 3.5 AC3 and the FR-13 test's real scope both still wait on Epic 4 surfaces that do not exist yet (`progress`/`groupForPrint`/`instantiateTemplate`) — the reviewer agreed with the existing deferrals already in `deferred-work.md`; no new action.
  - `[low]` `[reject]` Backspace/Delete chip removal does not check `isComposing`, and the Enter guard does not special-case Safari's legacy `keyCode 229` — this is the same already-deferred "virtual keyboard chip deletion unverified" risk in the spec's `deferred:` frontmatter; not re-logged separately.
- Verdict: approve to merge as-is (reviewer's own words), with the one medium item worth fixing if cheap — fixed above.
...
**Verification performed:** `docker compose --profile tools run --rm tools pnpm verify` run four times across this pass: (1) before the internal review's patches, (2) after them, (3) after merging `origin/main` (PR #20, the Porto Seguro fixture) — exit 0, kernel 514/514, web 571/571, tooling 20/20, api 105/105, e2e `@p0` 35/35 — and (4) after PR #21's independent review patches (the `Combobox` swap and three low fixes) — exit 0, kernel 514/514, web 572/572, tooling 20/20, api 105/105, e2e `@p0` 35/35 (including the Combobox-driven `3.5-E2E-001` and the extended `3.6-E2E-001`). Scoped `vitest`/`playwright -g`/eslint/tsc runs by the implementation subagent after each patch, per its reports above. Real-browser pass recorded above. Independent review (fresh context, PR #21, model opus): verdict "approve to merge as-is"; its one medium and three of its low findings were patched, the rest deferred or accepted as noted in the Review Triage Log.
