...
- `apps/web/src/copy/{pt-br,ui}.ts`, `src/styles/app.css`, `src/surfaces/registries/registries.css` -- Registries copy, `.frame-*` translations
...
- `e2e/cadastros.spec.ts` -- 4 tests: `@p0` core flow, `@p1` expired-sorts-first, `@p1` unreferenced Remover+undo, `@p2` offline create

**Review findings.** 10 findings from one combined review layer (blind hunt + edge cases + verification gap + intent alignment, per LEAN MODE): medium 6, low 4, false 0, maybe-false 0. Patched: 7 (Código made read-only once an instrument exists; `NumberField` reverts to the last committed value on an invalid blur instead of silently dropping it; a targeted `instrument-panel.test.tsx` added for AC4's referenced/unreferenced branch; four `calibrationCheck` window-boundary assertions added; `i-archive` wired into the Arquivar button; the panel's `aria-label` switched to `aria-labelledby` on its own heading). Deferred: 2 (broader component-test coverage for the remaining new surfaces; concurrent-edit reconciliation while a panel is open on two devices, a pre-existing Epic-1-wide pattern) -- both in `deferred-work.md` and this spec's frontmatter. Rejected: 2 (a blank "ghost" instrument creatable via a non-identifying field alone, consistent with the story's own no-required-fields boundary; the spec's Verification section listing expected rather than transcribed command output, addressed structurally by this run's own `pnpm verify`).

**Follow-up review recommendation:** true (first pass; 0 high patched, but 4 medium-severity entries were patched, over the "two or more medium" threshold). Named unverified risk: this pass used one combined review layer (LEAN MODE) instead of four independent ones, so a disagreement or gap a second, differently-angled layer might have caught cannot be ruled out; the mandatory independent review in the orchestrator playbook (a fresh subagent with no context from this run) is the next check on that risk.

**Verification.** `docker compose --profile tools run --rm tools pnpm verify` green (exit 0) after all patches: lint, static, domain 316 tests / 26 files, web 408 tests / 50 files (including the new `instrument-panel.test.tsx`), api 69 tests / 12 files (including the new cross-tenant case), Playwright `@p0` 15/15 including `2.1-E2E-001`; full run ~2 minutes. `pnpm test:e2e:full` was run once by the implementation subagent (31/31 green, including all 4 `e2e/cadastros.spec.ts` tests at their real priority tags) but not re-run after the review patches, since none of the patches touch e2e-covered paths beyond what `@p0`'s `2.1-E2E-001` already re-verified. Matrix audit: every I/O & Edge-Case Matrix row is covered by a passing test -- valid/expiring/expired/no-period-context/missing-data by `calibration.test.ts` (now including the boundary sweep), referenced-delete by `references.test.ts` plus the new `instrument-panel.test.tsx`, unreferenced-delete and offline-create by `e2e/cadastros.spec.ts`.

**Residual risks.** The AC4 referenced-instrument path still has no e2e coverage (no `block` can exist before Epic 5); it is covered at the kernel level and now at the component level, but the full Dexie-live-query wiring (`blockRows` -> `isInstrumentReferenced` -> panel prop) is only exercised by unit/component tests, not end to end. `rbc_accredited` and `calibration_interval_months` have no mock control to check placement/wording against; both carry `// authored:` copy notes flagged for design review. The manual real-browser pass (390/768/1280, light/dark, keyboard, offline) from this spec's own Verification section was not run by the implementation or review passes; the mandatory independent review (playbook step 3) covers this.
