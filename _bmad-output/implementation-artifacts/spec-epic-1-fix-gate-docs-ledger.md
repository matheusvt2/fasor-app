...
**Residual risks:** `test-reset.ts`'s CLI now rejects any company id outside the two `TEST_SEED` companies rather than resetting an arbitrary real company -- a deliberate, spec-directed narrowing (see deferred-work.md) rather than a defect.

**Merge status:** `origin/main` was merged into this branch (PR #9's identity-id work landed first; two conflicts resolved, see PR #10's comments). `pnpm verify` is fully green (`test:unit` x3, `test:api` x3, `test:e2e`, `lint`, `static`, all clean).

**Root cause of the `replay.integration.test.ts` failure, found and fixed:** it was neither a code defect in `apply.ts`/`packages/domain` nor caused by PR #9. `apps/api/src/db/schema.ts`'s `ops.op_id` is a globally unique key by design (`apply.ts`'s `ForeignOpIdError`, "an op_id that exists under another company can never be inserted" -- correct production behavior, since a real client mints effectively-random op_ids). `packages/domain/fixtures/replay-small/op-log.ts` deliberately reuses the *same fixed* op_ids on every test run (for a deterministic golden snapshot). `replay.integration.test.ts`'s old `afterAll` only cleaned up rows under *that run's own* fresh `companyId`; a run killed before its `afterAll` executes (a network interruption, a container restart -- this happened repeatedly during this session) leaves the fixture's fixed op_ids permanently owned by an abandoned company, so every later run's insert of those same op_ids is correctly rejected as belonging to someone else (`op_invalid`). This explained every observed symptom: identical failing op_ids across repeated runs on the same long-lived dev volume (the same rows, still there); the failure's absence on a genuinely fresh volume (nothing to collide with yet); and PR #9's own reported green `test:api` (its own fresh-ish volume at the time had no such leftovers).

Confirmed by reconciling the contradiction the coordinator raised: (a) `origin/main`'s api suite on a fresh volume passed `replay.integration.test.ts` but failed a *different* test instead (`seed.integration.test.ts`'s user-projection double-create, itself a known race -- already in the ledger); (b) this branch's suite (with `fileParallelism: false`, added earlier for the `resetTestCompanyData` race) passed all 68/68 tests on a fresh volume. Isolated the variable by adding *only* `fileParallelism: false` to an unmodified `origin/main` checkout: 66/66 green, twice. Then proved the actual root cause directly: planted a real orphaned `ops` row (fixed `op_id`, bogus company) on this branch's already-fixed volume -- test still passed (self-healed); temporarily disabled the new `beforeAll`, replanted the same orphan -- the exact original failure reproduced (`op_invalid`, one short of the expected count); re-enabled the fix -- green again.

**Fix applied** (smallest correct one, in test isolation, not production code): `apps/api/src/sync/replay.integration.test.ts` gained a `beforeAll` that deletes any existing `ops` rows matching this fixture's own op_ids, regardless of which company currently owns them, before the test claims them under its own fresh company. No assertion was touched or weakened. `fileParallelism: false` (already in this branch) independently prevents the *concurrent* variant of this class of problem (two files racing on shared TEST_SEED company state); this fix additionally makes the suite immune to *leftover* state from a previously aborted run, which `fileParallelism` alone does not clean up.
