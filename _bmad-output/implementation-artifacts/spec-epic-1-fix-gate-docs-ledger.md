...
**Residual risks:** the pre-existing `replay.integration.test.ts` failure (unrelated, out of the forbidden `apps/api/src/sync` / `packages/domain` paths); the `fileParallelism: false` stress-test gap noted above; `test-reset.ts`'s CLI now rejects any company id outside the two `TEST_SEED` companies rather than resetting an arbitrary real company -- a deliberate, spec-directed narrowing (see deferred-work.md) rather than a defect.

**Merge status:** `origin/main` was merged into this branch (PR #9's identity-id work landed first; two conflicts resolved, see PR #10's comments). The `replay.integration.test.ts` failure survives that merge unchanged -- it is not caused by, and was not fixed by, PR #9 either, confirmed by the independent reviewer checking out `origin/main` tip directly and by re-running `pnpm test:api` on this branch post-merge (same 2 failing tests, same op_ids). This blocks a fully green `pnpm verify` for any PR right now, not just this one. Per AGENTS.md's merge-gate policy (R-011), this PR is not squash-merged while `pnpm verify` is red; PR #10 stays open, fully implemented and independently approved, pending a human decision on the pre-existing `apps/api/src/sync`/`packages/domain` defect.
